...
function normalizeState(raw: any): ConnectStatus["connectionState"] {
  const s = String(raw?.instance?.state || raw?.state || "").toLowerCase();
  if (s === "open") return "CONNECTED";
  if (s === "connecting") return "CONNECTING";
  return "DISCONNECTED";
}

function extractQrBase64(raw: any): string | null {
  const b64 = raw?.base64 || raw?.qrcode?.base64 || null;
  if (!b64) return null;
  return b64.startsWith("data:") ? b64 : `data:image/png;base64,${b64}`;
}

function looksLikeMissingInstance(raw: any) {
  const msg = JSON.stringify(raw || {}).toLowerCase();
  return raw?.error || msg.includes("does not exist") || msg.includes("not found");
}

/**
 * Garante que a instância existe (criando se preciso), aponta o webhook pro
 * `/api/webhook/evolution` deste site, e devolve o estado real de conexão +
 * QR Code direto do motor Evolution API hospedado no Render.
 */
async function fetchStatus(): Promise<ConnectStatus> {
  let state = await getEvolutionState();
  if (looksLikeMissingInstance(state)) {
    await createEvolutionInstance();
    state = await getEvolutionState();
  } else {
    // Reafirma o webhook mesmo em instância já existente (idempotente),
    // pra garantir que aponta pra URL certa deste deploy.
    await setEvolutionWebhook().catch(() => {});
  }
  const connectionState = normalizeState(state);
  if (connectionState === "CONNECTED") {
    return { connectionState, qrCodeBase64: null };
  }
  const qr = await getEvolutionQRCode();
  return { connectionState, qrCodeBase64: extractQrBase64(qr) };
}

export async function GET(request: NextRequest) {
  const status = await fetchStatus().catch(
    (): ConnectStatus => ({ connectionState: "DISCONNECTED", qrCodeBase64: null }),
  );
            ...
            ${
              ...
              `
                ...
                <div class="qr-box" style="padding: 40px; color: #64748b;">
                  ⏳ <strong>Gerando QR Code...</strong> (se o Render estiver "dormindo", pode levar até 1 minuto)
                </div>
              `
            }
          </div>
      ...
      <div id="broadcastTab" class="tab-content">
        <div class="broadcast-grid">
          ...
          <div>
            ...
            <p style="color: #667781; font-size: 14px; margin-bottom: 20px;">Envie ofertas e cupons para a sua lista de contatos.</p>
