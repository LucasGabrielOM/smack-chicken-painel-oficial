...
export async function GET(request: NextRequest) {
  ...
  const [summary, hourly, days, payments, products, finance] = await Promise.all([
    ...
    query(`SELECT EXTRACT(HOUR FROM created_at)::int AS hour,COALESCE(SUM(total_cents),0)::int AS value
           FROM orders WHERE created_at::date=CURRENT_DATE AND status<>'cancelled'
           GROUP BY 1 ORDER BY 1`),
    query(`SELECT created_at::date AS day,COALESCE(SUM(total_cents),0)::int AS value,COUNT(*)::int AS orders
           FROM orders WHERE created_at>=CURRENT_DATE-INTERVAL '13 days' AND status<>'cancelled'
           GROUP BY 1 ORDER BY 1`),
    ...
    query(`SELECT
           ...
           FROM finance_entries WHERE date_trunc('month',entry_date)=date_trunc('month',CURRENT_DATE)`),
  ]);
